...
static MINECRAFT_SESSION: OnceLock<MinecraftSession> = OnceLock::new();
...
impl MinecraftSession {
    ...
    fn load_current_session(&self) -> Result<SessionInfo, String> {
        ...
        if let Ok(username_field) =
            env.get_field(session_obj.clone(), "field_74286_b", "Ljava/lang/String;")
        {
        ...
        if let Ok(player_id_field) =
            env.get_field(session_obj.clone(), "field_148257_b", "Ljava/lang/String;")
        {
        ...
        if let Ok(access_token_field) =
            env.get_field(session_obj.clone(), "field_148258_c", "Ljava/lang/String;")
        {
...
pub fn get_minecraft_session() -> &'static MinecraftSession {
    MINECRAFT_SESSION.get_or_init(|| {
        let mut session = MinecraftSession::new();
